...
const SearchableLanguageSelector: Component<SearchableLanguageSelectorProps> = (props) => {
  ...
  return (
    ...
    >
      ...
      <Combobox.Positioner>
        <Combobox.Content
          class="rounded-6px max-h-300px overflow-y-auto z-1000 shadow-[0_4px_12px_rgba(0,0,0,0.3)] py-4px w-max outline-none focus:outline-none"
          style={{
            'background-color':
        ...
        >
          <For each={collection().items}>
            {(language) => (
              ...
              >
                <Combobox.ItemText>{language.label}</Combobox.ItemText>
              </Combobox.Item>
            )}
          </For>
        </Combobox.Content>
